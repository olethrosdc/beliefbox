       ...
       random(random_), pit(pit_)
{
    ...
    SingularDistribution pit_reward(pit);
	...
	for (uint y=0; y<height; ++y) {
		...
		case GRID:
		case WALL:
		    mdp->setRewardDistribution(x + y*width, a, &step_reward);
		    break;
		case GOAL:
		    mdp->setRewardDistribution(x + y*width, a, &zero_reward);
		    break;
		case PIT:
		    mdp->setRewardDistribution(x + y*width, a, &pit_reward);
		    break;
		default:
