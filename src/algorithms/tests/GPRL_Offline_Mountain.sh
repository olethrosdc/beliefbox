...
do
	...
	mkdir -p $outdir
	cd $outdir
	sem -j 8 --id gprl_mutex GPRL --environment MountainCar --discount 0.999 --n_training_episodes $n_rollout --n_training_e_steps 40 --n_testing_episodes 1000 --n_testing_e_steps 1000 --grid 4 --scale 1.0 --seed $n_runs --seed_file $HOME/data/r1e7.bin 2>&1 >out.log
    done
