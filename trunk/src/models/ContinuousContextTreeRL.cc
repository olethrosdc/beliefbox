...
ContinuousContextTreeRL::Node::Node(int n_branches_,
                          int n_outcomes_)
...
real ContinuousContextTreeRL::Node::QValue(Ring<int>& history,
                                 Ring<int>::iterator x,
                                 real Q_prev)
{
    //w = exp(log_w_prior + log_w); 
 ...
 */
real ContinuousContextTreeRL::QLearning(real step_size, real gamma, int observation, real reward)
{
 ...
 */
real ContinuousContextTreeRL::Sarsa(real step_size, real gamma, int observation, real reward)
{
    real Q_prev = root->QValue(history, history.begin(), 0);
    //assert(!isnan(Q_prev));
...
///
/// x = (y, a)
real ContinuousContextTreeRL::QValue(int x)
{
