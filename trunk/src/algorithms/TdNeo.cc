...
TdNeo::TdNeo(const int n_states_,
...
real TdNeo::Observe(const int state, const int action, const real reward, const int next_state, const int next_action)
{
	...
	LAMBDA(state, action) = (mean == 0) ? DEFAULT_LAMBDA : ((deviation > mean) ? 0 : 1 - deviation / mean);
...
real TdNeo::Observe(const real reward, const int next_state, const int next_action)
{
	if(state >= 0)
	{
